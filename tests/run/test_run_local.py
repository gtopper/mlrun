...
def test_run_local_with_uid_does_not_exist(monkeypatch):
    ...
    spec = tag_test(base_spec, "test_run_local_with_uid_does_not_exist")
    function = mlrun.new_function(
        name="run-local-uid-does-not-exist",
        kind="job",
        command=f"{examples_path}/training.py",
    )
    result = function.run(spec, workdir=examples_path, local=True)
    verify_state(result)
